...
export function SpotlightInteractive({
  ...
  const getSpotlightGradient = () => {
    // Use a neutral gradient during SSR and initial render
    if (!mounted) {
      return "bg-[radial-gradient(circle_at_center,rgba(255,255,255,0)_0%,transparent_100%)]";
    }
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        mounted && resolvedTheme === "dark" ? "blur-sm" : "blur-[1px]",
        isHovered && mounted ? "opacity-100" : "opacity-0",
        className,
      )}
      style={{
        width: size,
