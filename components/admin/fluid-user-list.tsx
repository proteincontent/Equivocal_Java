  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div
          className={`relative w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold text-white shadow-lg ${
            isAdmin
        ...
        <div className="flex flex-col min-w-0">
          <span className="font-bold text-gray-800 truncate text-lg">{user.email}</span>
          <div className="flex items-center gap-2">
            <span
              className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                isAdmin ? "bg-purple-100 text-purple-600" : "bg-emerald-100 text-emerald-600"
              }`}
            >
              {isAdmin ? "管理员" : "普通用户"}
            </span>
    ...
    <div className="space-y-8">
          ...
          >
            ...
            {search && (
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-full text-sm font-medium">
                ...
                <span>
                  搜索 &ldquo;{search}&rdquo; 找到 {total} 个用户
                </span>
                <button
                  onClick={handleClearSearch}
            ...
            {roleFilter !== "all" && (
              ...
              >
                <span>{roleFilter === "admin" ? "仅显示管理员" : "仅显示普通用户"}</span>
                <button
                  onClick={handleClearFilter}
                  className={`ml-1 p-0.5 rounded-full transition-colors ${
                    roleFilter === "admin" ? "hover:bg-purple-100" : "hover:bg-emerald-100"
                  }`}
                >
                  <X className="w-3.5 h-3.5" />
                </button>
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <div className="text-sm text-gray-500">
          {total > 0 ? (
            <span>
              显示第 <span className="font-semibold text-gray-700 tabular-nums">{startIndex}</span>{" "}
              - <span className="font-semibold text-gray-700 tabular-nums">{endIndex}</span> 个， 共{" "}
              <span className="font-semibold text-gray-700 tabular-nums">{total}</span> 个用户
            </span>
          ) : (
            <span>暂无用户</span>
          )}
